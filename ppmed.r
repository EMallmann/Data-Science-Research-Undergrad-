...
newResiduals <- residuals(combinedFit, type = "pearson")
...
poorlyPred = which(abs(newResiduals) > median(abs(newResiduals)))
forRF = forRF[poorlyPred,]
...
poorlypredictedmedianROC = roc(curveResponse, impForest)
plot.roc(poorlypredictedmedianROC)
save(poorlypredictedmedianROC, file = "poorlypredictedmedianROC.RData" )
