...
for(i in 1:numSNPs) { #3 is to exclude pheno, binary, and ordinal
	fit = glm(Chr22geno$binary ~ Chr22geno[,i])
	fitSum = summary(fit)
...
combinedFit = glm(binary ~ . , data = forReg)
...
secondForest <- cforest(resid~., data = forRF, controls=cforest_unbiased(mtry=100, ntree=500))
